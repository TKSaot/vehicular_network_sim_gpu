...
def tx_pipeline(hdr_bytes: bytes, payload_bytes: bytes, cfg: SimConfig):
    ...
    mapped, map_meta = map_bytes(payload_bytes, cfg.byte_mapping, cfg.seed, frame_count=len(tmp_frames), mtu=cfg.mtu_bytes)
    ...
    all_symbols = []
    frame_metas = []
    preamble = build_preamble(cfg.preamble_len)
    pilots = build_qpsk_pilots(cfg.pilot_len)
    ...
    for fr in header_frames:
        ...
        syms = bits_to_symbols(inter, cfg.mod_scheme)
        symbols = np.concatenate([preamble, pilots, syms])
    ...
    concat = np.concatenate(all_symbols)
